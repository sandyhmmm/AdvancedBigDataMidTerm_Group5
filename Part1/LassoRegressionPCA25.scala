...
object LassoRegressionPCA25 {
  def main(args: Array[String]) {

    val conf = new SparkConf().setAppName("LassoRegressionPCA25")
...
//PCA for top 25
val pca = new PCA(25).fit(training2.map(_.features))
...
val rdg = new LassoWithSGD()
rdg.optimizer.setNumIterations(numIterations).
  setRegParam(0.01).
  setStepSize(0.0001)

val model = rdg.run(trainingProjected)
